...
- zodiac 必须写成两个自然段，全文 180–260 汉字。
...
- 可以适当的，自然的，参考元素、模式、守护星等占星特征。但不是必须，要在能深化当前分析时才使用，不要为了显得专业而罗列术语。

- 第一段：以当前问题为情境，重点分析这个星座最被触动的 1–2 个深层特质，并用牌面照见这些特质此刻如何表现。
- 第二段：根据牌面与问题进一步选择该星座另一项与当前问题最相关的深层特质。结合牌面分析这一特质会如何影响其看待、理解或经历眼前这件事，或者容易坚持、误判、回避、真正看重的地方。结尾仍然落回“这个星座在这件事中的独特反应”。如果牌面与当前问题确实呈现出值得用户看清、留意或重新理解的地方，可以自然点出；若没有必要，则直接收束分析，不必强行加入提醒或建议。避免延伸成泛化的人生建议。
